...
async function handleRequest(request) {
  ...
  // 构建最终字符串,生成 HTML 页面内容
  const finalStringss = [...uniqueStrings];


  const finalStrings = finalStringss.map(str => `${str}`).join('\n');

  const finalStringsss=btoa(finalStrings);
  const encoder=new TextEncoder();
  const utf8array=encoder.encode(finalStringsss);
  const htmlContent=String.fromCharCode(...utf8array)
